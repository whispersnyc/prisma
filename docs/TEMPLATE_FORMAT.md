# Prismo Template Format Specification

## Overview
Prismo templates use the `.prismo` file extension and support line-specific replacements with variable substitution.
...
- `{colorN}` - Full hex color **without #** (e.g., `{color0}` → `a1b2c3`)
...
```yaml
templates:
    simple-theme.prismo: ~/.config/app/theme.css
```
...
7. **Use @full for simple themes**: If you need complete file replacement, use `@full` in a `.prismo` template
